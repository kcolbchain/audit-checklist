...
| `custom-unchecked-call-detector` | Medium | High | Low-level `.call()`, `.delegatecall()`, or `.staticcall()` whose return values (success boolean) are not checked, leading to silent failures | ```solidity\nfunction withdraw() external {\n    msg.sender.call{value: bal}(""); // unchecked\n}\n``` | `slither . --detect custom-unchecked-call-detector` |
...
| `custom-unchecked-call-detector` | [`slither/detectors/custom_unchecked_call.py`](slither/detectors/custom_unchecked_call.py) |
